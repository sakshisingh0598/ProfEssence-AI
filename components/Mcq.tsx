...
const Mcq = ({ options, question, correctAnswer, questionIndex }: Props) => {
  ...
  return (
    <div className="p-1">
      <Card className="bg-orange-400/30 shadow-lg border-orange-500 border-[1px]">
        ...
        <CardContent className="flex flex-col items-center justify-center gap-3 w-full z-10">
          {options.map((option, optionIndex) => (
            <Button
              key={optionIndex}
              className={cn(
                "w-full h-auto bg-orange-300/70 hover:bg-orange-400 text-black whitespace-normal flex justify-start transition-none",
                {
                  "bg-orange-400/80 hover:bg-orange-500 text-white":
            ...
            >
              <div className="text-left flex">
                <p className="text-slate-500 mr-1">{optionIndex + 1}.</p>{" "}
                {option}
              </div>
            </Button>
          ))}
