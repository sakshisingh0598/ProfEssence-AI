...
export async function getQuiz(chatId: number) {
  ...
  try {
    const result = await generateObject({
      ...
      prompt: `Generate an array of multiple-choice questions (MCQs) (Do not use apostrophes for keys.) on the topic: ${data} no of questions: ${amount}. shuffle the correct option index, Return the output in JSON format. Ensure that only the values are enclosed in double quotes (""), and the keys are not enclosed in any quotes. Use the following format for the JSON object:
      [
        {
